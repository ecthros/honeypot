...
def kick(name):
	...
	os.system("vzctl exec " + str(name) + " who awk \'!/root/{ cmd=\"/sbin/pkill -KILL -u \" $1; system(cmd)}\'")#removes all users.
...
def recycle(name):
	...
	os.system("vzctl create " + str(name) + " --ostemplate ubuntu")#Template is named ubuntu
